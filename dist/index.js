...
const server = new Server({ name: "forge-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
    ...
    try {
        switch (name) {
            case "search_forge_docs": {
                ...
                const version = args?.minecraft_version;
                const results = await searchService.searchDocs(query, category, version);
                if (results.results.length === 0) {
                    return { content: [{ type: "text", text: `No results for "${query}".` }] };
                }
                const formatted = results.results.map((r, i) => `## ${i + 1}. ${r.title} (${r.minecraft_version})\n**Category:** ${r.category}\n**URL:** ${r.url}\n\n${r.content}\n`).join("\n---\n\n");
            ...
            case "get_example": {
                ...
                const formatted = results.examples.map((ex, i) => `## ${i + 1}. ${ex.title}\n\`\`\`${ex.language}\n${ex.code}\n\`\`\`\n`).join("\n---\n\n");
