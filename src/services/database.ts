...
export class DatabaseService {
  ...
  private async downloadDatabase(): Promise<void> {
    ...
    try {
      ...
      const asset = releaseInfo.assets?.find((a: any) => a.name === "forge-docs.db");
      if (asset) {
  ...
  private fetchReleaseInfo(): Promise<any> {
    return new Promise((resolve, reject) => {
      https.get(
        "https://api.github.com/repos/Hayremyt/mcp-minecraft-forge/releases/latest",
        { headers: { "User-Agent": "forge-mcp" } },
        (res) => {
          let data = "";
