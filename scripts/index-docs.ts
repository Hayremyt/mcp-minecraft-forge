...
async function fetchPage(url: string): Promise<string | null> {
  ...
  while (true) {
    ...
    try {
      ...
      if (response.ok) {
        if (attempt > 1) console.log(`  OK attempt ${attempt}`);
        return await response.text();
      }
      console.log(`  Fail attempt ${attempt}: HTTP ${response.status}`);
    } catch (err) {
      console.log(`  Fail attempt ${attempt}: ${err instanceof Error ? err.message : 'error'}`);
    }
...
async function main() {
  ...
  db.run(`CREATE TABLE documents (id INTEGER PRIMARY KEY AUTOINCREMENT, url TEXT UNIQUE NOT NULL, title TEXT NOT NULL, content TEXT NOT NULL, category TEXT NOT NULL, minecraft_version TEXT NOT NULL, hash TEXT NOT NULL);`);
  db.run(`CREATE TABLE chunks (id TEXT PRIMARY KEY, document_id INTEGER NOT NULL, chunk_type TEXT NOT NULL, content TEXT NOT NULL, section_heading TEXT, code_language TEXT, word_count INTEGER, has_code INTEGER DEFAULT 0);`);
  db.run(`CREATE VIRTUAL TABLE documents_fts USING fts5(title, content, content='documents', content_rowid='id');`);
  db.run(`CREATE INDEX idx_doc_version ON documents(minecraft_version);`);
  db.run(`CREATE INDEX idx_doc_category ON documents(category);`);
  ...
  for (const version of FORGE_VERSIONS) {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      db.run("INSERT OR REPLACE INTO documents (url, title, content, category, minecraft_version, hash) VALUES (?, ?, ?, ?, ?, ?)",
        [url, p.title, content.substring(0, 10000), p.category, version, hash]);

      const docResult = db.exec("SELECT id FROM documents WHERE url = ?", [url]);
      const docId = docResult[0]?.values[0]?.[0] as number;
      if (!docId) { console.log("  NO DOC ID"); continue; }

      db.run("INSERT OR REPLACE INTO chunks VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [docId + "-t", docId, "title", p.title, p.title, null, 0, 0]);
      if (p.description) db.run("INSERT OR REPLACE INTO chunks VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [docId + "-d", docId, "description", p.description, "Description", null, 0, 0]);
      for (const s of p.sections) {
        if (s.content.trim()) { db.run("INSERT OR REPLACE INTO chunks VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [docId + "-s" + totalSections, docId, "section", s.content.substring(0, 3000), s.heading, null, 0, 0]); totalSections++; }
        for (const c of s.code) { const wc = c.split(/\s+/).length; db.run("INSERT OR REPLACE INTO chunks VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [docId + "-c" + totalCode, docId, "code", c.substring(0, 3000), p.title + " - " + s.heading, "java", wc, 1]); totalCode++; }
      }
      vd++; totalDocs++;
